...
CGFloat const XZRedTimelineCollectionViewCellMarginLeftRight = 5;
...
+(instancetype)cellWithTableView:(UITableView *)tableView withDataArray:(NSArray *)dataArray andIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell.data.count > 0) {
        [cell.redTimelineCollectionView reloadData];
    }
    return cell;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    XRedTimelineCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:XZRedTimelineCellIdentifier forIndexPath:indexPath];
...
//    [cell.contentView addSubview:label];
    ...
    [cell.imgView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",IMGURL,dict[@"imgUrl"]]] placeholderImage:nil];
