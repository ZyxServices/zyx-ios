...
-(id)initWithFrame:(CGRect)frame withDataArray:(NSArray *)dataArray {
    self = [super initWithFrame:frame];
    if (self) {
        NSMutableArray *array = [NSMutableArray array];
        for (NSDictionary *dict in dataArray) {
            [array addObject:[NSString stringWithFormat:@"%@%@",IMGURL,dict[@"imgUrls"]]];
        }
        
        NSMutableArray *labelArray = [NSMutableArray array];
        for (NSDictionary *dict in dataArray) {
            [labelArray addObject:dict[@"title"]];
        }
        SDCycleScrollView *cycleScrollView = [SDCycleScrollView cycleScrollViewWithFrame:CGRectMake(0, 0, XZScreenWidth, 180) shouldInfiniteLoop:YES imageNamesGroup:array];
        cycleScrollView.delegate = self;
        cycleScrollView.pageControlStyle = SDCycleScrollViewPageContolStyleClassic;
        cycleScrollView.titlesGroup = labelArray;
        [self addSubview:cycleScrollView];
        cycleScrollView.scrollDirection = UICollectionViewScrollDirectionHorizontal;
        //         --- 轮播时间间隔，默认1.0秒，可自定义
        //cycleScrollView.autoScrollTimeInterval = 4.0;
